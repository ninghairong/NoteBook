...
public class DiaryDao {
    ...
    public long insert(Diary diary) {
        ...
        cv.put(DBContants.TABLE_FIELD_PHOTO_PATH,diary.getPhoto());
        //        long result2 = 1;
//
//
//        /*
...
//            return result1;
//        }
        return db.insert(DBContants.TABLE_DIARY_NAME, null, cv);
        // 失败
 //       return -1;
    }

    public Cursor searchByWord(String word){
        SQLiteDatabase db = helper.getReadableDatabase();
        String sql = "select * from " + DBContants.TABLE_DIARY_NAME+" where "+DBContants.TABLE_FIELD_CONTEXT+" like '" +word+"' or "+DBContants.TABLE_FIELD_TITLE+" like '"+word+"'";
...
//        }
        // 失败
        return db.update(DBContants.TABLE_DIARY_NAME, cv, DBContants.TABLE_FIELD_ID + "=?", new String[]{diary.getId().toString()});
    ...
    public long delete(Integer diaryID){
        ...
        return db.delete(DBContants.TABLE_DIARY_NAME,DBContants.TABLE_FIELD_ID+"=?",new String[]{diaryID.toString()});
    }
}
